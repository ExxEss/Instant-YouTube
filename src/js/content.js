...
(function () {
  if (
    document.location.href.includes("youtube") ||
    document.location.href.includes("bilibili")
  )
    return;
  ...
  const insertPlayButtons = () => {
    if (
      document.getElementsByClassName("instantYoutubeViewCount").length === 0 &&
  ...
  const removeVideo = () => {
    if (
      document.getElementsByClassName("instantYoutubeVideoContainer").length > 0
    ) {
      saveVideoFrame(() => {
        closeLogo.style.display = "none";
        iframe.src = null;
  ...
  window.onload = () => {
    ...
    if (
      document.getElementsByClassName("instantYoutubeButtonContainer")
        .length === 0
    )
      insertPlayButtons();
  };
  ...
  window.setInterval(() => {
    if (
      !Util.isSelectable(document.activeElement) &&
